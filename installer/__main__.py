...
def createApp (connect):
    appname = "misakitgbot" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
if __name__ == "__main__":
    ...
    if os.path.isdir("./misakiuserbot/"):
        rm_r("./misakiuserbot/")
    repo = Repo.clone_from(slicedString,"./misakiuserbot/", branch="master")
    ...
    config['DEFAULT_BIO'] = "✨ @MisakiUserBot ✨ "
    config['DEFAULT_NAME'] = "Misaki sahip"
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/ByMisakiMey/MisakiUserBot"
    config['WARN_LIMIT'] = "5"
    ...
    console.print(Panel(f"\n[bold green]{LANG['END'].format(round(time() - baslangic))}[/]\n", title="✨ 𝐌𝐢𝐬𝐚𝐤𝐢 𝐔𝐬𝐞𝐫𝐁𝐨𝐭 ✨", border_style="bold green"), justify="center")
